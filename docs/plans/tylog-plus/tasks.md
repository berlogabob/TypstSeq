...
Contract: [plan.md](plan.md). Updated 2026-09-26. Coordinator owns this file.
...
**Current snapshot (2026-09-26):** bounded host primitives are accepted for P05,
...
| P12 | Paged startup/list reads | P11 | METRICS CORRECTED / BUILD FRAME GATES FAIL | Startup/open/save pass. Stage-budget measurement now uses the observed display refresh rate; the historical 21.4% total-span figure was not a dropped-frame rate. Corrected five-minute plain run: 83/1,098 stage violations (7.56%) at 90 Hz; formatted smoke: 56/56 at 120 Hz. UI/build remediation and editing acceptance remain; [latest evidence](evidence/P12e/result.md#stage-budget-measurement-correction-2026-09-23) |
...
| P18 | PDF reader/versioned extraction | P07,P14 | MAC CORPUS & IMAGE RENDER PASS / PASSWORD + MEMORY OPEN | Five-PDF private Mac corpus opened; no-text encrypted image page rasterized; password-required PDFs and memory threshold remain open |
...
| P21i query embedder bridge | Coordinator | DONE | P21f | [Query bytes are validated and fused with stored vectors in one tested helper](evidence/P21/result.md) |
...
| P18f private no-text page rendering | Coordinator | DONE | P18e | [The corpus's no-text encrypted PDF renders non-background page pixels in PDFium](evidence/P18/result.md#redacted-private-no-text-page-render-check-2026-09-25) |
...
| P22h Mac real-vault route | Coordinator + local Ornith | PERFORMANCE FAIL / INVESTIGATION OPEN | P22d | [Read-only 6,298-note route opens a selected note with zero vault writes; 100 samples exceed 500 ms p95 and Flutter cannot foreground the app](evidence/P22/mac-real-vault-open-100-samples.json) |
...
| P12e latency acceptance | Coordinator | STARTUP/SAVE PASS / EDITOR ACCEPTANCE OPEN | P12a-P12d | Re-evaluate stage-budget timing separately from frame latency; retain startup/open/save evidence |
| P12f editor mode gate | Coordinator | ROUTE VERIFIED / TIMING RECHECK | P12e | 46 KB / 220-block SAF fixture selects stock TextField; historical p95 39.01 ms is total latency, not a stage-budget verdict |
| P12g virtualized block editor | Coordinator | INTERACTION SMOKE / TIMING RECHECK | P12f | Visible rows and basic typing/Enter/Backspace/undo smoke recorded; historical total-span counts do not establish missed frames or full cross-row editing parity |
...
Model routing (2026-09-24/25): user requested local Ollama `ornith-1.5:9b` for coding
and other suitable tasks. Its P22 test draft (2,940 prompt / 1,904 output tokens)
needed correction. A second small sampling helper draft (313/309 tokens) also
needed coordinator correction before execution. The measurement wrapper and
assertions were reviewed and verified locally; native checks remain the
acceptance authority. Unload local inference before performance measurement.
