...
#let brand = rgb("#1F2937") // headings, table header fill, primary marks
#let accent = rgb("#0EA5A4") // ONE accent — rules, active marks, positive state
#let ink = rgb("#1F2937") // body text
#let muted = rgb("#6B7280") // secondary text, captions, footer
#let hairline = rgb("#E5E7EB") // rules, borders
#let surface = rgb("#F9FAFB") // callout / tile fill
#let zebra = rgb("#FBFBFC") // alternating table row
...
#let body-font = ("Inter", "Noto Sans Symbols 2")
...
#let report(meta, doc) = {
  ...
  show heading: set block(above: 1.5em, below: 0.75em, sticky: true)
...
// ---------------------------------------------------------------- TABLES
// Editorial data table: no vertical rules, hairline horizontals, zebra rows,
// first column left / rest right, repeating header, optional footer row.
// headers: array of strings. rows: array of arrays of content/strings.
...
// Pandoc's Typst writer emits `#divider()` for Markdown thematic breaks.
// Keeping it here lets source-faithful research annexes compile unchanged.
#let divider() = line(length: 100%, stroke: 0.6pt + hairline)
