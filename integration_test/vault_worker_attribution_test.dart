...
''',
    ...
    // Stand-ins for the controller's retained state.
    VaultIndex? retained;
    final searchTarget = PkmsSearchIndex.empty();
    ...
    await for (final event in worker.run(
      ...
      switch (event) {
        ...
        case PkmsBuiltEvent(:final search):
          closePhase('PkmsBuilt->done');
          replace.start();
          searchTarget.replaceWith(search);
          replace.stop();
    ...
    expect(replace.elapsedMicroseconds, greaterThan(0));
    ...
    // ignore: avoid_print
    print('  sync searchIndex.replaceWith:${replace.elapsedMilliseconds}ms');
  });
}
